...
export default async function handler(req, res) {
    ...
    const { userId, score } = req.body; // Inclure la localisation dans les données

    // Validation des données d'entrée
    if (!userId || typeof score !== "number") {
        console.error("Requête invalide : userId, score", { userId, score });
    ...
    try {
        ...
        const updateResult = await db.collection("users").updateOne(
            { _id: objectId },
            {
                ...
                $push: { scores: { score } }, // Ajouter le score et la localisation dans l'historique des scores
            }
        );
        ...
        console.log("Score inséré dans scores :", insertResult);

        // Ajouter également le score dans la collection `FlagScore` si c'est le jeu de drapeaux
        const flagScoreInsertResult = await db.collection("FlagScore").insertOne({
            userId: userId,
            username: user.username,
            score,
        });

        console.log("Score inséré dans FlagScore :", flagScoreInsertResult);
